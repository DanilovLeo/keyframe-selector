...
def build_extractor(
    ...
    name must be one of:
        "uniform", "velocity_zero", "gripper_state", "gripper_fallback", "awe", "random"
    """
    ...
    elif name == "gripper_fallback":
        ext = GripperFallbackExtractor(min_n=gripper_fallback_min_n, gripper_min_dist=gripper_min_dist)
    elif name == "awe":
        ext = AWEExtractor(error_threshold=awe_eps)
    elif name == "random":
        ext = RandomExtractor(n_keyframes=random_n, seed=random_seed)
    else:
        raise ValueError(
            f"Unknown extractor '{name}'. "
            "Choose: uniform | velocity_zero | gripper_state | gripper_fallback | awe | random"
        )
